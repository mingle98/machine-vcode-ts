...
export class collectionData extends CollectionData {
    ...
    async postData(type: 'verify' | 'during' | 'destroy'= 'during', verData?: any, callback?: Function, failFn?: Function) {
        try {
            ...
            if (+(<PostData>res).data.code === 0 && +(<PostData>res).status === 200) {
                let tk = this.collectionDataStore.verifyData.rzData.tk;
                tk !== (<PostData>res).data.data.tk && (this.collectionDataStore.verifyData.rzData.tk = (<PostData>res).data.data.tk);
