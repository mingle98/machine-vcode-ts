...
import { CollectionDataOptions, CollectionDataStore, GatherEventStore, PostData } from "../../interface/collectionData";
...
export default abstract class CollectionData{
    ...
    protected collectionDataStore: CollectionDataStore = {
        ...
        postUrl: globalConfig.collectionDataConfig.openLocalUrl ? globalConfig.collectionDataConfig.localpostUrl : globalConfig.collectionDataConfig.postUrl,
        postInitUrl: globalConfig.collectionDataConfig.openLocalUrl ? globalConfig.collectionDataConfig.localpostInitUrl : globalConfig.collectionDataConfig.postInitUrl,
        // 是否初始化成功
    ...
    protected listenEventArr: (keyof typeof ListenEventArrStore)[] = ['mousedown', 'mouseenter', 'mouseleave', 'mouseup', 'mousemove', 'mouseover', 'mousewheel', 'click', 'touch', 'touchstart', 'touchmove', 'touchend'];
    ...
    protected initData(options?: CollectionDataOptions) {
        ...
        this.postInitData((data: PostData['data']) => {
    ...
    async postInitData(callback?: Function, failFn?: Function) {
        try {
            let res = await utils.$https.request({
                url: this.customCollectionDataOptios.open ? this.customCollectionDataOptios.initpostUrl : this.collectionDataStore.postInitUrl,
                method: 'post',
                data: {
                    type: 'init',
    ...
    protected initGatherEvent() {
        let doc = document;
        let win = window;
        let addEventHandler = this.baseCls.addEventHandler.bind(this.baseCls);
    ...
    removeGatherEvent() {
        let doc = document;
        let win = window;
        let removeEventHandler = this.baseCls.removeEventHandler.bind(this.baseCls);
    ...
    protected getScreenInfo() {
        try {
            ...
            let clientHeight = doc.documentElement.clientHeight || doc.body.clientHeight;
            // 屏幕分辨率
            let screenWidth = win.screen.width;
            let screenHeight = win.screen.height;
            // 屏幕可用工作区
            let availWidth = win.screen.availWidth;
            let availHeight = win.screen.availHeight;
            // 外部高度
            let outerWidth = win.outerWidth;
            let outerHeight = win.outerHeight;
            // 文档高度 网页内容实际宽高(包括工具栏和滚动条等边线）
            let scrollWidth = doc.documentElement.scrollWidth || doc.body.scrollWidth;
            let scrollHeight = doc.documentElement.scrollWidth || doc.body.scrollHeight;
            return {
                'screenTop': screenTop,
    ...
    protected gatherEventStore(type: keyof GatherEventStore) {
        let ownDataStore = this.collectionDataStore;
        let me = this;
        let doc = document;
        let gatherEventStore: GatherEventStore = {
    ...
    protected resetDzData() {
        let rzData = this.collectionDataStore.verifyData.rzData;
