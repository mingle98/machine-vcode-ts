...
const myhttp = 'http://101.43.139.11:90';
const imghttp = 'https://luckycola.com.cn/public';
// let myhttp = 'http://localhost:90';
// let localhttp = location.protocol + '//' + location.host;
const localhttp = myhttp;
// let localhttp = 'http://localhost:90';
const luckColahttp = 'http://luckycola.com.cn';
...
const configObj = {
  ...
  getConfigImgsfromServer: (
    fromServer: boolean = false,
    customImgsConfig?: { url: string; method: string }
  ) => {
  ...
  syncImgReq: async (customImgsConfig: { url: string; method: string }) => {
    try {
      const tresData: any = await utils.$https.request({
